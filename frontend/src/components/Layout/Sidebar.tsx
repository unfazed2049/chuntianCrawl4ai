    ...
    <aside className="border-r border-white/60 bg-slate-950 text-slate-100 lg:sticky lg:top-[97px] lg:h-[calc(100vh-97px)]">
      <div className="flex h-full flex-col px-4 py-6">
        <div className="mb-6 rounded-3xl border border-white/10 bg-white/5 p-4">
          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-sky-300/80">Navigation</p>
          <p className="mt-3 text-sm leading-6 text-slate-300">
            从侧栏切换不同情报视图，主内容区会保留更适合阅读的卡片与摘要结构。
          </p>
        </div>
              ...
              >
                <span
                  className={cn(
                    ...
                    isActive ? 'bg-white/15' : 'bg-white/6 group-hover:bg-white/12'
                  )}
                >
                ...
                <span className="min-w-0">
                  ...
                  <span className="mt-1 block text-xs leading-5 text-inherit/75">{item.description}</span>
                </span>
              </button>
            );
          })}
